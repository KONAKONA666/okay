...
class ModelStep_6 extends Model {
    ...
    public function admin_conf() {
        ...
        if(!is_writable($this->current_dir.'/cml/.passwd')) {
            $this->result['errors'][] = $this->get_translation('error_write_file') . ' ' . $this->current_dir . '/cml/.passwd';
        }
        if(!is_writable($this->current_dir.'/cml/.htaccess')) {
            $this->result['errors'][] = $this->get_translation('error_write_file') . ' ' . $this->current_dir . '/cml/.htaccess';
        }
        ...
        if(isset($_POST['admin_conf'])) {
            ...
            if(empty($this->result['errors'])) {
                ...
                if(mysqli_query($this->dbc, "INSERT INTO ".$this->config->db_prefix."managers SET id=1, login='".$this->login."', password='".$encpassword."'")) {
                    $path_to_passwd = $this->current_dir.'/cml/.passwd';
                    $passstring = $this->login.':'.$encpassword;
                    $passfile = fopen($path_to_passwd, 'w');
                    fputs($passfile, $passstring);
                    fclose($passfile);
                    $htaccess = file_get_contents($this->current_dir.'/cml/.htaccess');
                    $htaccess = preg_replace("/AuthUserFile .*\n/i", "AuthUserFile $path_to_passwd\n", $htaccess);
                    $htafile = fopen($this->current_dir.'/cml/.htaccess', 'w');
                    fwrite($htafile, $htaccess);
                    fclose($htafile);
                    
                    if(!empty($this->email)) {
                        mysqli_query($this->dbc, "UPDATE ".$this->config->db_prefix."settings SET value='".$this->email."' WHERE name='order_email' OR name='comment_email' OR name='notify_from_email' OR name='admin_email'");
                    }
                    if(!empty($this->phone)) {
                        mysqli_query($this->dbc, "INSERT INTO ".$this->config->db_prefix."settings SET name='admin_phone', value='".$this->phone."'");
                    }
